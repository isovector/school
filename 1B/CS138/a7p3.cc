...
/*
    int val = 0;
    int bs[8] = {2, 7, 3, 5, 11, 13, 5, 2};
    int mo[8] = {8, 2, 3, 3, 8, 7, 7, 5};
    int pi[18] = {3, 1, 4, 1, 5, 9, 2, 6, 5, 3, 5, 8, 9, 7, 9, 3, 2, 3};

    for (int r = 0; r < 8; r++) {
        for (int i = 0; i < key.length(); i++) {
            val += key[i] * i;
            //val ^= ~(key[pi[i] % key.length()]) | pi[i];
            val <<= bs[r] * i % mo[r];
            val ^= key[i];
        }
        val *= bs[r] + mo[r];
        val += (int)(sin((val % 360) * M_PI / 180) * getTableSize());
    }
    if (val < 0)
        if (val % 2 == 1)
            val *= -1;
            ...
            val = ~val;
    return val % nextLowerPrime(getTableSize());*/
}
